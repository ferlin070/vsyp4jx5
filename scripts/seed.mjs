#!/usr/bin/env node
/**
 * seed — realistic starter data for the current app.
 *
 * Writes DESIGN.md (from designs/, picked by keyword) if missing, and
 * src/seedData.ts + a test. Data is domain-aware (finance/ecommerce/task/
 * generic — detected from the brief text or the PRD if present) so records
 * match the guard generated by `npm run brief`. Never overwrites existing
 * files.
 *
...
import { detectDomain, detectByGuard, seedDataTemplate, SEED_TEST } from './lib/domains.mjs';
...
// Domain detection: ground truth = the guard already in src/schema.ts,
...
if (!existsSync(seedPath)) {
  writeFileSync(seedPath, seedDataTemplate(domain, count));
...
console.log('      2. Seed on first run: if (loaded.length === 0) store.save(seedItems())');
